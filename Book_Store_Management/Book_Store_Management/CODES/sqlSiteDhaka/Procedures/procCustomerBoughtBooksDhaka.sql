CREATE OR REPLACE PROCEDURE customerBoughtBooksDhaka
	(var_custNum in number) IS

cursor c is select cust_name, order_date, book_title from (select order_num, order_date, cust_num from orders@site_link_dhaka) ord 
	...
	inner join (select book_num, book_title from books@site_link_dhaka) bo on bo.book_num = ordd.book_num;
